...
import { isEmpty as _isEmpty, create, apply, applyFormat, split, toHTMLString, getTextContent, insert, insertLineSeparator, isEmptyLine, unstableToDom, getSelectionStart, getSelectionEnd, remove, isCollapsed } from '@wordpress/rich-text';
  ...
  _createClass(RichText, [{
    ...
    value: function onKeyDown(event) {
      ...
      if (keyCode === DELETE || keyCode === BACKSPACE) {
        ...
        var end = getSelectionEnd(value);

        if (keyCode === BACKSPACE) {
          this.onChange(remove(value, // Only remove the line if the selection is
          // collapsed.
          isCollapsed(value) ? start - 1 : start, end));
        } else {
          this.onChange(remove(value, start, // Only remove the line if the selection is collapsed.
