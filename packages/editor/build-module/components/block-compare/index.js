  ...
  _createClass(BlockCompare, [{
    ...
    value: function getConvertedContent(block) {
      ...
      var newContent = newBlocks.map(function (item) {
        return getSaveContent(getBlockType(item.name), item.attributes, item.innerBlocks);
      });
      var renderedContent = newBlocks.map(function (item) {
        return getSaveElement(getBlockType(item.name), item.attributes, item.innerBlocks);
      });
      return {
