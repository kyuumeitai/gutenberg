...
function (_Component) {
  ...
  _createClass(HierarchicalTermSelector, [{
    ...
    value: function onChange(event) {
      ...
      var newTerms = hasTerm ? without(terms, termId) : _toConsumableArray(terms).concat([termId]);
    ...
    value: function onAddTerm(event) {
      ...
      if (existingTerm) {
        ...
        if (!some(terms, function (term) {
          ...
          onUpdateTerms(_toConsumableArray(terms).concat([existingTerm.id]), taxonomy.rest_base);
        }
      ...
      findOrCreatePromise.then(function (term) {
        ...
        onUpdateTerms(_toConsumableArray(terms).concat([term.id]), taxonomy.rest_base);
      }, function (xhr) {
    ...
    value: function render() {
      ...
      var filterLabel = sprintf(_x('Search %s', 'term'), get(this.props.taxonomy, ['name'], slug === 'category' ? __('Categories') : __('Terms')));
      var groupLabel = sprintf(_x('Available %s', 'term'), get(this.props.taxonomy, ['name'], slug === 'category' ? __('Categories') : __('Terms')));
