...
export function isCurrentPostPublished(state) {
  var post = getCurrentPost(state);
  return ['publish', 'private'].indexOf(post.status) !== -1 || post.status === 'future' && moment(post.date).isBefore(moment());
}
/**
...
export function isEditedPostBeingScheduled(state) {
  var date = moment(getEditedPostAttribute(state, 'date')); // Adding 1 minute as an error threshold between the server and the client dates.
